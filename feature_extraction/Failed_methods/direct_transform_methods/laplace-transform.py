...
BW=freq<20000
ROC=np.linspace(-4900,-5000,101)
ans=np.zeros((ROC.size, np.fft.rfft(signal,n=signal.size*pad_reps).size))
for i,theta in enumerate(ROC):
    new_signal=np.multiply(signal,np.exp(-theta*time))
    spectrum=np.abs(np.fft.rfft(new_signal,n=signal.size*pad_reps))
    ans[i,:]=spectrum
    if theta==-4969.0:
        plt.figure('fft at pole')
        plt.plot(freq, spectrum)
    if theta<-4900:
        print(theta)
        plt.figure('new_signal')
